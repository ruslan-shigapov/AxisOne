...
struct SubgoalSorter {
    ...
    static func compare(lhs: Subgoal, rhs: Subgoal, for date: Date) -> Bool {
        if let lhsRawValue = TimesOfDay.getComparableValue(
            from: date,
            for: lhs),
           let rhsRawValue = TimesOfDay.getComparableValue(
            from: date,
            for: rhs),
        ...
        if let lhsRawValue = LifeAreas(rawValue: lhs.goal?.lifeArea ?? ""),
           let rhsRawValue = LifeAreas(rawValue: rhs.goal?.lifeArea ?? "") {
            return lhsRawValue.order < rhsRawValue.order
        }
        return false
    }
